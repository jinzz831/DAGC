...
class EvalDatasetVideoMME(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        # file_name: str = "test_graph_only.parquet",
        # file_name: str = "test-00000-of-00001_non_graph.parquet",
    ) -> None:
...
class EvalDatasetMLVU(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        all_data_list = dict(data_list)
        all_json_data = {}
        for task_name, task_spec in all_data_list.items():
            with open(os.path.join(data_path, task_spec[0]), "r") as f:
                all_json_data[task_name] = json.load(f)
        self.filter_before_count = sum(len(items) for items in all_json_data.values())
        self.requested_subset = mlvu_subset

        if mlvu_subset is not None:
            subset_alias = {
                "count": "count",
                "ego": "ego",
                "needle": "findNeedle",
                "findneedle": "findNeedle",
                "find_needle": "findNeedle",
                "order": "order",
                "plotqa": "plotQA",
                "plot_qa": "plotQA",
                "anomaly": "anomaly_reco",
                "anomaly_reco": "anomaly_reco",
                "topic": "topic_reasoning",
                "topic_reasoning": "topic_reasoning",
            }
            subset_norm = str(mlvu_subset).strip()
            subset_key = subset_alias.get(subset_norm.lower(), subset_norm)
            if subset_key not in data_list:
                raise ValueError(f"Unknown MLVU subset: {mlvu_subset}")
            data_list = {subset_key: data_list[subset_key]}

        list_data_dict = []
        for k, v in data_list.items():
            json_data = all_json_data[k]
        ...
        self.data = list_data_dict
        self.filter_after_count = len(self.data)
        self.selected_task_types = sorted(data_list.keys())
